...
int FSpeechRecoginzer::sr_init(struct speech_rec * sr, const char * session_begin_params, enum sr_audsrc aud_src, int devid, struct speech_rec_notifier * notifier)
{
	...
	if (sr->session_begin_params == NULL) {
		UE_LOG(XunFeiLog, Error, TEXT("mem alloc failed!!!"));
		return -E_SR_NOMEM;
	}
	...
	if (aud_src == SR_MIC) {
		...
		if (sr->recorder == NULL || errcode != 0) {
			UE_LOG(XunFeiLog, Error, TEXT("create recorder failed: %d!!!"), errcode);
		...
		if (errcode != 0) {
			UE_LOG(XunFeiLog, Error, TEXT("recorder open failed: %d!!!"), errcode);
			errcode = -E_SR_RECORDFAIL;
			goto fail;
		}
	}
...
int FSpeechRecoginzer::sr_start_listening(struct speech_rec *sr)
{
	...
	if (sr->state >= SR_STATE_STARTED) {
		UE_LOG(XunFeiLog, Log, TEXT("Already STARTED!!!"));
		return -E_SR_ALREADY;
	}
	...
	{
		UE_LOG(XunFeiLog, Error, TEXT("QISRSessionBegin failed! error code:%d!!!"), errcode);
		return errcode;
	}
	...
	if (sr->aud_src == SR_MIC) {
		...
		if (ret != 0) {
			UE_LOG(XunFeiLog, Error, TEXT("start record failed: %d!!!"), ret);
...
int FSpeechRecoginzer::sr_stop_listening(struct speech_rec *sr)
{
	...
	if (sr->state < SR_STATE_STARTED) {
		UE_LOG(XunFeiLog, Warning, TEXT("Not started or already stopped!!!"));
		return 0;
	}
	...
	if (sr->aud_src == SR_MIC) {
		...
		if (ret != 0) {
			UE_LOG(XunFeiLog, Error, TEXT("Stop failed!!!"));
			return -E_SR_RECORDFAIL;
		}
	...
	if (ret != 0) {
		UE_LOG(XunFeiLog, Error, TEXT("write LAST_SAMPLE failed: %d!!!"), ret);
	...
	while (sr->rec_stat != MSP_REC_STATUS_COMPLETE) {
		...
		if (MSP_SUCCESS != ret) {
			UE_LOG(XunFeiLog, Error, TEXT("QISRGetResult failed! error code: %d!!!"), ret);
